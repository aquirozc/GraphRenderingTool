...
public class FRGraphRenderer {
    ...
    public static void computeVertexPositions(Graph graph, int w, int h){
        double temp = w /10;
        double k = (double) Math.sqrt((w*h)/graph.vertexes.size());
        ...
        for(double i = 0; i < 50; i++){
            for (Vertex v : graph.vertexes){
                ...
                for(Vertex u : graph.vertexes){
                    if(!v.equals(u)){
                        ...
                        double dlen = (double) Math.sqrt(d1*d1 +d2*d2);
            ...
            for(Vertex v : graph.vertexes){
                ...
                v.updatePos((double)Math.min(w, Math.max(0, v.getPosX())), (double)Math.min(h, Math.max(0, v.getPosY())));
            }

            temp = (double) Math.max(temp * (1 - 0.065), 1);
        }
    }
